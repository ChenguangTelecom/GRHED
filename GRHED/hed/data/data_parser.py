...
class DataParser():
    ...
    def get_batch(self, batch):
        ...
        for idx, b in enumerate(batch):
            ...
            im=sio.loadmat(self.samples[b][0])['image_data']
            em=sio.loadmat(self.samples[b][1])['gt_data']
  
            im = cv2.resize(im,(self.cfgs['training']['image_width'], self.cfgs['training']['image_height']))
            im -= self.cfgs['mean_pixel_value']
            im=im[:,:,1:5]

            em = cv2.resize(em,(self.cfgs['training']['image_width'], self.cfgs['training']['image_height']))
